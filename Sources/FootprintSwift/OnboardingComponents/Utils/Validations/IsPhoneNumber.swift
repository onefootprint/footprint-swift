...
struct PhoneNumberValidator {
    ...
    static func isMobilePhone(_ value: String, options: [String: Any] = [:])  -> Bool {
        if let sandbox = options["sandbox"] as? Bool, sandbox {
            if IsSandboxFixtureNumber.isSandboxFixtureNumber(value) {
                return true
            }
        }
        
        if let locales = options["locale"] as? [String] {
            return locales.contains { locale in
                if let pattern = phones[locale] ?? phones[aliases[locale] ?? ""] {
